...
if [ -f /etc/version ]; then
    router_type=$(cat /etc/version | cut -d . -f 1)
fi
...
while IFS= read -r line; do
    ...
    if $inside_wireguard_config; then
        if [[ $router_type == "EdgeRouter" ]]; then
            ...
            if [[ $line =~ description\ ([a-zA-Z0-9.-]+) ]]; then
        ...
        if [[ $line == *"}"* ]]; then
            ...
            if [[ -n $peer_name && $peer_name =~ [a-zA-Z] ]]; then
                ...
                new_ip=$(host -4 "$peer_name" | grep -o "IPv4 address.*" | awk '{print $3}' | tail -n 1)
